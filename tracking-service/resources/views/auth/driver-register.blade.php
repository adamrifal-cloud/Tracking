<x-app-layout>
    <div class="min-h-screen flex flex-col md:flex-row-reverse bg-[#FFC107] md:bg-white w-full">
        ...
        <div class="md:w-1/2 bg-[#FFC107] flex flex-col items-center justify-center pt-16 pb-20 md:py-12 px-8 relative overflow-hidden">
            ...
            <!-- Back Button -->
            <a href="{{ route('driver.login') }}" class="absolute top-6 left-6 md:top-8 md:left-8 text-white hover:text-gray-100 transition-colors z-20">
            ...
            <div class="hidden md:block text-center text-white mt-8 z-10">
                ...
                <p class="text-lg text-yellow-100 max-w-sm mx-auto">Daftar sebagai mitra kurir, kelola pengiriman dengan mudah, dapatkan rute terbaik, dan lacak performa secara instan.</p>
            </div>
        </div>
        ...
        <div class="md:w-1/2 bg-white flex flex-col justify-center px-8 py-10 md:px-16 lg:px-24 rounded-t-[3rem] md:rounded-none -mt-10 md:mt-0 relative z-20 flex-1 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] md:shadow-none">
            ...
            <div class="max-w-md w-full mx-auto">
                <div class="text-center md:text-left mb-10">
                    <h2 class="text-3xl font-extrabold text-gray-900 mb-2">Driver Register</h2>
                    <p class="text-gray-500">Lengkapi data diri pendaftaran kurir baru</p>
                </div>
                ...
                <form action="{{ route('driver.register.post') }}" method="POST" class="space-y-5">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5 ml-4">Nama Lengkap</label>
                        <input type="text" name="name" required placeholder="Cth. Alex Rider" value="{{ old('name') }}"
                            class="w-full px-6 py-3.5 rounded-full bg-gray-50 text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FFC107] focus:bg-white border border-gray-200 transition-all font-medium text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5 ml-4">Email Address</label>
                        <input type="email" name="email" required placeholder="Cth. alex@driver.com" value="{{ old('email') }}"
                            class="w-full px-6 py-3.5 rounded-full bg-gray-50 text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FFC107] focus:bg-white border border-gray-200 transition-all font-medium text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5 ml-4">Nomor Telepon</label>
                        <input type="text" name="phone" required placeholder="Cth. 08123456789" value="{{ old('phone') }}"
                            class="w-full px-6 py-3.5 rounded-full bg-gray-50 text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FFC107] focus:bg-white border border-gray-200 transition-all font-medium text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5 ml-4">Password</label>
                        <input type="password" name="password" required placeholder="••••••••"
                            class="w-full px-6 py-3.5 rounded-full bg-gray-50 text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FFC107] focus:bg-white border border-gray-200 transition-all font-medium text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5 ml-4">Konfirmasi Password</label>
                ...
                <div class="mt-8 text-center pb-8 md:pb-0">
                    <p class="text-gray-600 text-sm">
                        Sudah punya akun? 
                        <a href="{{ route('driver.login') }}" class="font-bold text-[#FFB703] hover:text-[#e5a403] transition-colors">Masuk di sini</a>
                    </p>
                </div>
            </div>
